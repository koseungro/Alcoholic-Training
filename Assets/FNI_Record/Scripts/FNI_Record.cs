...
namespace FNI
{
    ...
    public class FNI_Record : MonoBehaviour
    {
        ...
        /// 녹음기가 켜져있는지 확인합니다.
        /// </summary>
        public bool IsShow { get { return m_canvas.activeSelf; } }
    ...
    = 44100;
        ...
        /// <summary>
        /// 녹음 패널이 보여지거나 숨겨질 대상입니다.
        /// </summary>
        private GameObject m_canvas;
        /// <summary>
        /// 그로우가 기준으로 삼을 대상입니다.
        /// </summary>
        private RectTransform m_statePannel;

        /// <summary>
        /// 시간이 적힐 곳입니다.
        /// </summary>
        private Text m_time;
        public Text RecordingTime { get => m_time; }
        /// <summary>
        /// 음성 모양 아이콘입니다.
        /// </summary>
        ...
        /// 녹음 패널에 번져나갈 이미지 입니다.
        /// </summary>
        private Image m_grow;
        /// <summary>
        /// 그로우 애니메이션의 현재 진행 시간입니다.
        /// </summary>
        ...
        void Start()
        {
            ...
            m_statePannel = m_canvas.transform.Find("State").GetComponent<RectTransform>();

            //m_title = m_state.Find("Title").GetComponent<Text>();
            m_time = m_statePannel.Find("Time").GetComponent<Text>();
            m_icon = m_statePannel.Find("Icon").GetComponent<Image>();

            m_grow = m_statePannel.Find("GrowMask/Grow").GetComponent<Image>();
            m_amount = m_statePannel.Find("Amount").GetComponent<Image>();

            m_start = m_canvas.transform.Find("Start").GetComponent<Button>();
            m_stop = m_canvas.transform.Find("Stop").GetComponent<Button>();
            m_replay = m_canvas.transform.Find("Replay").GetComponent<Button>();

            m_start.onClick.AddListener(Start_Record);
            //m_stop.onClick.AddListener(Stop_Record);
            m_replay.onClick.AddListener(Replay);
        ...
        public void Show(RecordType type = RecordType.Default, float second = 0, bool isActiveStart = false, bool isStopAndHide = false)
        {
            ...
            //기타 값 초기화
            m_amount.fillAmount = 0;
            m_icon.rectTransform.sizeDelta = m_sIconSize;
            m_time.text = "00:00";
        ...
        public void RecordReady(RecordType type = RecordType.Default, bool interactable = true)
        {
            ...
            m_replay.gameObject.SetActive(type == RecordType.Replay);
            if (type == RecordType.Replay)
                m_replay.interactable = interactable;
            else
                m_replay.interactable = false;

            m_stop.gameObject.SetActive(false);
            m_start.gameObject.SetActive(true);
            m_stop.interactable = false;
        }
        ...
        public void Start_Record()//녹음 시작
        {
            ...
            if (m_type != RecordType.TimeLimit)
            {
                m_stop.interactable = true;
                m_stop.gameObject.SetActive(true);
            }
        ...
        private IEnumerator AnimationRecord_Routine(bool isStart)
        {
            ...
            if (isStart)
            {
                ...
                m_time.color = new Color(1, 1, 1, 1);
            }
            else
            {
                m_time.color = new Color(1, 1, 1, 1);
            ...
            while (sTime < eTime)
            {
                ...
                m_icon.rectTransform.sizeDelta = isStart ? Vector2.Lerp(m_sIconSize, m_eIconSize, percent) :
                                                          Vector2.Lerp(m_eIconSize, m_sIconSize, percent);
                m_icon.rectTransform.anchoredPosition = isStart ? Vector2.Lerp(m_sIconPos, m_eIconPos, percent) :
                                                                 Vector2.Lerp(m_eIconPos, m_sIconPos, percent);
                //m_time.color = isStart ? Color.Lerp(new Color(1, 1, 1, 0), new Color(1, 1, 1, 1), percent) :
                //                        Color.Lerp(new Color(1, 1, 1, 1), new Color(1, 1, 1, 0), percent);
                yield return null;
            }
            ...
            m_icon.rectTransform.sizeDelta = isStart ? m_eIconSize : m_sIconSize;
            m_icon.rectTransform.anchoredPosition = isStart ? m_eIconPos : m_sIconPos;
            //m_time.color = isStart ? new Color(1, 1, 1, 1) : new Color(1, 1, 1, 0);
        }

        /// <summary>
        /// 녹음이 시작되면 주변으로 그로우를 표현합니다.
        /// </summary>
        private void EffectGrow()
        {
            //비활성화 일 경우 활성화를 한다.
            if (m_grow.gameObject.activeSelf == false)
            {
                m_curT = 0;
                m_grow.gameObject.SetActive(true);
            }

            if (m_curT < m_growTime)
            {
                //그로우 애니메이션을 작동합니다.
                m_curT += Time.deltaTime;
                float percent = m_curT / m_growTime;

                if (percent < m_moveStart)//m_moveStart이하의 값에서 그로우가 나타나게 합니다.
                {
                    float _percent = percent / m_moveStart;
                    m_grow.color = new Color(m_grow.color.r, m_grow.color.g, m_grow.color.b, Mathf.Lerp(0, 1, _percent));
                    m_grow.rectTransform.sizeDelta = m_statePannel.sizeDelta + m_sGrowSize;
                }
                else//m_moveStart이상의 값에서 그로우가 퍼지게 합니다.
                {
                    float _percent = (percent - m_moveStart) / (1 - m_moveStart);
                    m_grow.color = new Color(m_grow.color.r, m_grow.color.g, m_grow.color.b, Mathf.Lerp(1, 0, _percent));
                    m_grow.rectTransform.sizeDelta = Vector2.Lerp(m_statePannel.sizeDelta + m_sGrowSize, m_statePannel.sizeDelta + m_eGrowSize, _percent);
                }
            }
            else
                m_curT = 0;
        ...
        private void SetTimeText()
        {
            if (IsRecording)//녹음 중 일 때
            {
                ...
                if (m_type == RecordType.TimeLimit || m_type == RecordType.SoftTimeLimit)//시간 제한의 경우, 지정 시간 -> 00:00
                {
                    ...
                    m_amount.fillAmount = (float)((DateTime.Now - m_startRercordTime).TotalSeconds / m_countDownTime.TotalSeconds);
                ...
                m_time.text = string.Format("{0:00}:{1:00}", progress.Minutes, progress.Seconds);

                EffectGrow();
            }
            else//녹음 중이 아닐 때
            {
                m_time.text = string.Format("{0:00}:{1:00}", 0, 0);
                if (m_grow.gameObject.activeSelf)
                    m_grow.gameObject.SetActive(false);
            }
        }
