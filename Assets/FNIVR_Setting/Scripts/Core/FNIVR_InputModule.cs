...
/// 용  도: EventSystem의 InputModule 대체 용도
...
{
    public class FNIVR_InputModule : PointerInputModule
    {        
        [Header("Physics")]
        ...
        [NonSerialized]
        public FNIVR_GraphicRaycaster activeGraphicRaycaster;
        [Header("Dragging")]
        ...
        private void ProcessMousePress(MouseButtonEventData data)
        {
            ...
            if (data.PressedThisFrame())
            {
                ...
                if (pointerEvent.IsVRPointer_FNI())
                {
                    pointerEvent.SetSwipeStart_FNI(Input.mousePosition);
                }
        ...
        protected void CopyFromTo(FNIVR_PointerEventData @from, FNIVR_PointerEventData @to)
        {
        ...
        protected Dictionary<int, FNIVR_PointerEventData> m_VRRayPointerData = new Dictionary<int, FNIVR_PointerEventData>();


        protected bool GetPointerData(int id, out FNIVR_PointerEventData data, bool create)
        {
            if (!m_VRRayPointerData.TryGetValue(id, out data) && create)
            {
                data = new FNIVR_PointerEventData(eventSystem)
                {
                    pointerId = id,
                };
        ...
        virtual protected MouseState GetGazePointerData()
        {
            ...
            FNIVR_PointerEventData leftData;
            GetPointerData(kMouseLeftId, out leftData, true);
            leftData.Reset();

            //Now set the world space ray. This ray is what the user uses to point at UI elements
            leftData.worldSpaceRay = FNIVR_GazePointerSurpport.Instance.GetRay;
            ...
            FNIVR_GraphicRaycaster _raycaster = raycast.module as FNIVR_GraphicRaycaster;
            ...
            if (_raycaster)
            {
                ...
                leftData.position = _raycaster.GetScreenPosition(raycast);
                ...
                if (graphicRect != null)
                {
                    ...
                    FNIVR_GazePointer.instance.SetPosition(worldPos, normal);
                    // Make sure it's being shown
                    FNIVR_GazePointer.instance.RequestShow();
                }
            }

            // Now process physical raycast intersections
            FNIVR_PhysicsRaycaster physicsRaycaster = raycast.module as FNIVR_PhysicsRaycaster;
            if (physicsRaycaster)
            {
                ...
                if (performSphereCastForGazepointer)
                {
                    ...
                    physicsRaycaster.Spherecast(leftData, results, FNIVR_GazePointer.instance.GetCurrentRadius());
                ...
                {
                    FNIVR_GazePointer.instance.SetPosition(position, raycast.worldNormal);
                }
                else
                {
                    FNIVR_GazePointer.instance.SetPosition(position);
                }
            }
            ...
            FNIVR_PointerEventData rightData;
            GetPointerData(kMouseRightId, out rightData, true);
            CopyFromTo(leftData, rightData);
            rightData.button = PointerEventData.InputButton.Right;

            FNIVR_PointerEventData middleData;
        ...
        protected MouseState GetCanvasPointerData()
        {
            ...
            if (activeGraphicRaycaster)
            {
                ...
                FNIVR_Raycaster ovrRaycaster = raycast.module as FNIVR_Raycaster;
        ...
        */
        ...
        {
            var pressed = FNIVR_Input_Support.Instance.IsDown;
            var released = FNIVR_Input_Support.Instance.IsUp;
