...
/// 용  도: GraphicRaycaster 대체 역할
/// 사용법: Cavas에 GraphicRaycaster를 제거하고 FNIVR_GraphicRaycaster를 붙일 것
/// 수정이력 
/// 
...
public class FNIVR_GraphicRaycaster : GraphicRaycaster, IPointerEnterHandler
{
    ...
    protected FNIVR_GraphicRaycaster()
    { }
    ...
    public override void Raycast(PointerEventData eventData, List<RaycastResult> resultAppendList)
    {
        ...
        {
            Raycast(eventData, resultAppendList, eventData.GetRay_FNI(), true);
        }
    }
    /// <summary>
    ...
    private void DropGraphicRaycast(Canvas canvas, Ray ray, List<RaycastHit> results)
    {
    ...
    private void GraphicRaycast(Canvas canvas, Ray ray, List<RaycastHit> results)
    {
        ...
        for (int i = 0; i < foundGraphics.Count; ++i)
        {
            ...
            if (graphic.GetComponent<Dropdown>())
            {
                if (graphic.transform.Find("Dropdown List"))
                {
                    GraphicRaycast(graphic.transform.Find("Dropdown List").GetComponent<Canvas>(), ray, results);
                }
            }

            Vector3 worldPos;

            if (eventCamera && RayIntersectsRectTransform(graphic.rectTransform, ray, out worldPos))
            {
                //Work out where this is on the screen for compatibility with existing Unity UI code
                Vector2 screenPos = eventCamera.WorldToScreenPoint(worldPos);
                // mask/image intersection - See Unity docs on eventAlphaThreshold for when this does anything
                ...
                {
                    RaycastHit hit;
                    //Debug.DrawRay(eventCamera.gameObject.transform.position, worldPos - eventCamera.gameObject.transform.position, Color.red, 0.1f);
                    hit.graphic = graphic;
                    hit.worldPos = worldPos;
                    hit.fromMouse = false;
                    s_SortedGraphics.Add(hit);
    ...
    /// <summary>
    /// Raycast가 RectTransform과 교차하는지 확인하고 교차한다면 교차점의 world position도 반환하는지 여부를 확인한다.
    /// </summary>
    ...
    static bool RayIntersectsRectTransform(RectTransform rectTransform, Ray ray, out Vector3 worldPos)
    {
        ...
        //Debug.DrawLine(corners[0], corners[3], Color.green, 0.1f);
    ...
    /// <summary>
    /// 현재의 input module과 같은 Raycaster인지 확인한다.
    /// </summary>
    /// <returns></returns>
    public bool IsFocussed()
    {
        FNIVR_InputModule inputModule = EventSystem.current.currentInputModule as FNIVR_InputModule;
        return inputModule && inputModule.activeGraphicRaycaster == this;
    }

    public void OnPointerEnter(PointerEventData e)
    {
        if (e.IsVRPointer_FNI())
        {
