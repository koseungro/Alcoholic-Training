...
public class VideoPlayerForSequence : MonoBehaviour, IVisualObject
{
    ...
    bool IVisualObject.IsFinish => true;

    [SerializeField] SZ_VideoPlayer MainVideo = null;

    bool isMainHmdPlay = false;

    public void Init()
    {
        FNIVR_HMDManager.hmdPlayAction += HmdPlayEvent;
    ...
    private void VideoEvent(SZ_VideoPlayer video, VideoOption option)
    {
    ...
    private void HmdPlayEvent()
    {
        ...
        if ((MainVideo.Duration - 1) > MainVideo.Time && isMainHmdPlay)
